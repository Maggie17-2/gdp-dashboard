...
# df_sheet2 = pd.read_excel(file_path, sheet_name='2024-01-01~2024-07-27意图分组1')
...
#     # 左边的柱状图
#     axes[0].bar(x - width / 2, calls, width=width, label='通话数', color='blue')
#     axes[0].bar(x + width / 2, chats, width=width, label='对话数', color='orange')
#     axes[0].set_xticks(x)
#     axes[0].set_xticklabels(months)
#     axes[0].legend()
#
#     for y in range(0, max(max(calls), max(chats)) + 50, 50):
#         axes[0].axhline(y=y, color='gray', linestyle='--', linewidth=0.5)
#
#     # 右边的折线图
#     axes[1].plot(months, direct_rate, marker='o', label='直接转人工率', color='blue')
#     axes[1].plot(months, consult_rate, marker='o', label='咨询后转人工率', color='orange')
#     axes[1].yaxis.set_major_formatter(PercentFormatter(1))
#     axes[1].legend()
#     for y in np.arange(0, max(max(direct_rate), max(consult_rate)) + 0.05, 0.05):
#         axes[1].axhline(y=y, color='gray', linestyle='--', linewidth=0.5)
#
#     # 调整布局
#     plt.tight_layout()
#
...
# TOP分类标签
# QRV转人工趋势
# render_header_and_date_selector("TOP15分类标签", [datetime.date(2024, 7, 22), datetime.date(2024, 7, 28)],
#                                 "select_box4", "date_input4", show_date_selector=True)
...
# 计算数量前10项的占比
# top_tags = df_filtered.nlargest(10, "数量").copy()
top_tags = df_filtered.nlargest(15, "数量").copy()
# top_tags["占比"] = top_tags["数量"] / total_quantity * 100
